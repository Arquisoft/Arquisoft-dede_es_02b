  ...
  function acciones(): JSX.Element {
    if (sessionStorage.getItem("usuario"))
      if (JSON.parse(sessionStorage.getItem("usuario")!).esAdmin) {
        ...
        return (
          <Box sx={{ display: 'flex', flexDirection: "row-reverse", alignItems: 'center' }}>
            <IconButton id={"addtocart_" + productProp.product.nombre.toLowerCase()} type="submit" onClick={handleClick}>
              <AddShoppingCart />
            </IconButton>
            <Typography sx={{ fontSize: 20 }}> {accounting.formatMoney(producto.precio, "€")}</Typography>
            <IconButton id={"addunit_" + productProp.product.nombre.toLowerCase()} onClick={() => sumarCantidad(1)}><AddIcon /></IconButton>
            <TextField
            ...
            />
            <IconButton id={"removeUnit_" + productProp.product.nombre} onClick={() => sumarCantidad(-1)}><RemoveIcon /></IconButton>
